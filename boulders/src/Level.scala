...
final case class Level (width: Int, height: Int, leftWalls: Vector[Vector[Boolean]], floors: Vector[Vector[Boolean]],
                        boulders: Vector[Vector[Boolean]], start: GridPoint, diamond: GridPoint, exit: GridPoint)

object Level {
  /** The default level before a proper one is loaded. */
  val uninitiated: Level = empty (width = 3, height = 3, start = GridPoint (0, 2),
    diamond = GridPoint (0, 0), exit = GridPoint (1, 1))

  /** Creates an empty level of a given width and height, start, diamond and exit positions. */
  def empty (width: Int, height: Int, start: GridPoint, diamond: GridPoint, exit: GridPoint): Level =
    Level (width, height, emptyBooleanMatrix (width, height), emptyBooleanMatrix (width, height),
      emptyBooleanMatrix (width, height), start, diamond, exit)
  ...
  def decodeLevels (codes: String): Vector[Level] =
    codes.split ('\n').map (levelFromCode).toVector

  /** Decodes a single serialised level spec. */
  def levelFromCode (code: String): Level = {
    ...
    val empty = Level.empty (width, height, player, diamond, exit)
