...
case object FlipIndicator    extends Indicator
case object ReplayIndicator  extends Indicator

case class TutorialGuideLine(
    indicator: Indicator,
    text1: String,
    text2: String,
    continue: String,
    enabled: Set[PlaySceneButtonEvent]
)

object TutorialGuideLine {
  def loadGuide(kind: LevelKind, guide: String): Vector[TutorialGuideLine] =
  ...
  def loadLine(line: String): TutorialGuideLine = {
    ...
    val continue = indicator match {
      ...
      case FlipIndicator    => "Click flip/press F"
    ...
    val enabled: Set[PlaySceneButtonEvent] = indicator match {
      ...
      case FlipIndicator    => Set(BackButtonEvent, FlipButtonEvent)
